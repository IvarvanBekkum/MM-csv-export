...
class App:
	def __init__(self,parent):
		...
		self.frame.pack(padx=2,pady=2)
   		...
   		self.entry = Label(self.frame,width=110, text="This script will save the track points of the currently selected tracks as CSV file.")
	...
	def go(self, filename):
		...
		with codecs.open(filename, "w") as csvfile:
			...
			writer.writerow(['No', 'Latitude', 'Longitude', 'Time', 'Elevation', 'Speed'])
			...
			for track in myTracks.selectedTracks():
				...
				for point in track.points():
					...
					speed = point.speed()
					writer.writerow([count, latitude, longitude, time.strftime(timeFormat, timestamp).encode("utf-8"), elevation, speed])
